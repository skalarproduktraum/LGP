...
class EffectiveProgramInstructionGenerator<TProgram, TOutput : Output<TProgram>> : InstructionGenerator<TProgram, TOutput> {
    ...
    fun generateInstruction(effectiveRegisters: List<RegisterIndex>, branch: Boolean = false): Instruction<TProgram> {
        val outputs = when {
            effectiveRegisters.isEmpty() -> listOf(this.registers.calculationRegisters.first as RegisterIndex)
